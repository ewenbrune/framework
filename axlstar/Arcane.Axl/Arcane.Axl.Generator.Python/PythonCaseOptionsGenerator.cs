...
namespace Arcane.Axl
{
   ...
   * Classe générant le code des classes CaseOptions en C#.
   */
  class PythonCaseOptionsGenerator : PythonCodeGenerator
  {
    /**
    ...
    public PythonCaseOptionsGenerator(string path,
                                string output_path,
                                string name,
      ...
      : base(path, output_path)
    {
      ...
      if (!String.IsNullOrEmpty(namespace_macro_name))
        Console.WriteLine("namespace-macro-name is not used in Python generator");
    ...
    public override void writeFile()
    {
      ...
      PythonClassDefinitionGenerator cdg = new PythonClassDefinitionGenerator(file_stream, indent_level);
      foreach( Option opt in m_option_info_list)
        opt.Accept(cdg);
      ...
      /*
      {
        ...
        PythonOptionBuilderGenerator obg = new PythonOptionBuilderGenerator(file_stream,"ConfigList()",
                                                                            "new Arcane.XmlNode()");
        foreach(Option opt in m_option_info_list)
          opt.Accept(obg);

        PythonBuilderBodyGenerator bbg = new PythonBuilderBodyGenerator(file_stream);
        foreach(Option opt in m_option_info_list)
          opt.Accept(bbg);
      ...
      PythonVariableDefinitionGenerator vdg = new PythonVariableDefinitionGenerator(file_stream);
      foreach( Option opt in m_option_info_list){
        opt.Accept(vdg);
      }
      ...
      // write file
      _writeClassFile(m_name + "_axl.py", file_stream.ToString(), true);
    }
  }
}
